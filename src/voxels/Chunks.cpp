...
void Chunks::set(int x, int y, int z, int id, uint8_t states){
	...
	// Register set action in the network session
	if(NetSession *s = NetSession::GetSessionInstance())
	{
		...
		msg.usr_id = 0;
		msg.states = states;
		msg.coordinates.x = x + ox * CHUNK_W;
		msg.coordinates.y = y;
		msg.coordinates.z = z + oz * CHUNK_D;
		s->RegisterMessage(msg);
	}
}
